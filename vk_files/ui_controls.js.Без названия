 ...
 */
var UiControlsIcons = (function(){
  function getIcon16Cancel() {
    return {
      icon: '<svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg"><g fill="none" fill-rule="evenodd"><path d="M0 0h16v16H0z"/><path d="m8 6.73 3.46-3.47a.9.9 0 1 1 1.28 1.28L9.27 8l3.47 3.46a.9.9 0 1 1-1.28 1.28L8 9.27l-3.46 3.47a.9.9 0 1 1-1.28-1.28L6.73 8 3.26 4.54a.9.9 0 0 1 1.28-1.28L8 6.73z" fill="currentColor" fill-rule="nonzero"/></g></svg>',
      name: 'cancel_16',
    };
  }

  return {
    getIcon16Cancel: getIcon16Cancel,
  };
})();
...
function getBaseDomain() {
  let domain = ((window.vk && window.vk.__domain) || 'vk.com');
  if (!['vk.com', 'vk.ru'].includes(domain)) {
    domain = 'vk.com';
  }
  return '//' + domain + '/';
}
...
extend(UiControl.prototype, {
  ...
  utils: {
    // TODO: импортировать из static/js/modules/web/helpers/html_entities.js
    // ! Изменения дублировать в обе функции, пока не реализован импорт.
    normalizeHTMLEntities: function (input) {
      return input.replace(
        /&(amp|#38|#x26);(amp|quot|nbsp|lt|gt|apos|deg|sup1|sup2|sup3|reg|copy|#[0-9]{2,5}|#x[a-f0-9]{2,4});/gi,
        '&$2;'
      );
    }
  }
...
createChildClass('Selector', UiControl, {
  ...
  defaultOptions: {
    ...
    progressBar: false,
    imageId: undefined,
    noImageSrc: 'https://vk.com/images/question_s.gif',
    hrefPrefix: 'id',
    useHrefBaseDomain: false, // use BASE_DOMAIN for url instead of relative url
    noBlur: false,
    zeroDefault: false,
    ...
    /**
     * @deprecated
     * For setting placeholder color use CSS custom property --ui-selector-placeholder on your Selector/Dropdown container
     */
    ...
    chooseFirst: false, // ???
    onItemActiveHandler: function(value) {},
    onItemEmptyHandler: function() {},
         ...
         <td class="selector">\
           <div class="placeholder_wrap1">\
             <div class="placeholder_wrap2">\
               <div class="placeholder_content" aria-hidden="true" id="selector_placeholder_content"></div>\
               <div class="placeholder_cover"></div>\
             </div>\
           </div>' + scrollFaderTop + '\
            <span class="selected_items" role="list" aria-label="' + this.options.ariaLabel + '"></span>\
            '+ scrollFaderBottom +'\
            <input type="text" class="selector_input" ' + this.readOnly + ' aria-labelledby="selector_placeholder_content"/>\
  ...
  updateInput: function() {
    ...
    if (!this.options.autocomplete && this.options.multiselect && this._selectedItems.length) {
      ...
      this.input.style.width = Math.max(23, w) + 'px';
      if (_parent) {
        if (_next) {
  ...
  showImage: function(itemValue, itemData) {
    ...
    if (itemData !== undefined && typeof(itemData[3]) == 'string' && itemData[3].length) {
      if (itemData[3] == 'none'){
        ...
        const baseDomain = this.options.useHrefBaseDomain ? getBaseDomain() : '/';
        img.parentNode.href = baseDomain + this.options.hrefPrefix + itemData[0]; // hack
        removeEvent(img.parentNode, 'click');
        this.options.onItemActiveHandler(itemValue);
      }
    } else {
      img.style.display = '';
      ...
      this.options.onItemEmptyHandler();
    }
    return true;
  },
  ...
  addTagData: function(data) {
    ...
    var tokenName = clean(unclean(stripHTML(data[1])));
    var removeLabel = clean(getLang('global_delete')) + ' ' + tokenName;
    var ariaRemoveBtnAttrs = ' tabindex="0" aria-label="' + removeLabel + '" ';
    var cancelIcon = UiControlsIcons.getIcon16Cancel().icon;
    token.innerHTML = tokenPrefix + '<span class="token_inner"><span class="x SelectorToken__x" ' + ariaRemoveBtnAttrs + '>' + cancelIcon + '</span><span class="l"><span class="lc">' + tokenName + '</span>' + (data[5] ? data[5] : '') + '</span>';
    ...
    if (this.options.limitedListHeight) {
      ...
      animate(this.selectedItemsContainer, {scrollTop: this.selectedItemsContainer.scrollHeight});
    }
  ...
  highlightTag: function (id) {
    ...
    animate(this.selectedItemsContainer, {scrollTop: token.offsetTop - this.selectedItemsContainer.clientHeight/2});
  ...
  request: function(success, failure) {
    var requestUrl = this.dataURL;

    if (this.frozenDataUrl) {
      requestUrl = this.frozenDataUrl;
    }
    ...
    if (this.options.al) {
      ...
      var url = requestUrl + sep + 'str=' + encodeURIComponent(term);
  ...
  setFrozenURL: function(url) {
    if (typeof(url) === 'string' && !this.frozenDataUrl) {
      this.frozenDataUrl = url;
    }
  },
  ...
  // Aria methods
  _initAria: function () {
    this.options.noBlur = true; // Для корректной работы ридеров фокус должен оставаться на элементе.
  ...
  /**
   * Обновление aria-selected для вариантов дропдауна.
   *
   * @param updateNotSelected - Обновлять ли свойства для невыбранных вариантов.
   * @private
   */
  ...
  _ariaOnSelectShow: function () {
    attr(this.input, 'aria-haspopup', true);
  },
  /**
   * Возвращает фокус к элемену, использую якорь.
   * Нужен т.к. в ряде ситуаций некоторые ридеры могут скидывать фокус на начало страницы после, например,
   * выбора значения в выпадащем списке.
   *
   * @private
   */
  ...
  /**
   * Создать якорь для возвращаения фокуса.
   * Нужен т.к. в ряде ситуаций некоторые ридеры могут скидывать фокус на начало страницы после, например,
   * выбора значения в выпадащем списке.
   * @private
   */
  ...
  /**
   * Просталяет атрбут aria-activedescendant для корректной работы с автодополнением.
   * @param el
   * @private
   */
...
createChildClass('Select', UiControl, {
  ...
  content: function(items) {
    ...
    for (index = 0; index < len; ++index) {
      ...
      var encodedValue;
      if (partConfigEnabled && partConfigEnabled('ui_controls_normalize_html_entities')) {
        encodedValue = this.utils.normalizeHTMLEntities(value);
      } else {
        encodedValue = value
          .replace(/&amp;/g, '&')
          .replace(/&quot;/g, '"')
      ...
      var encodedText;
      if (partConfigEnabled && partConfigEnabled('ui_controls_normalize_html_entities')) {
        encodedText = this.utils.normalizeHTMLEntities(text.replace(/<[^>]+>/g, ''));
      } else {
      ...
      html.push(
        ...
        (index ? '' : this.CSS.FIRST) + '" role="button" id="' + optId + '">',
        text,
        '</li>'
      );
    }
  ...
  /**
   * Установить фокус на текущий выбранный эленмент для спец. возможностей.
   * Фокус устанавливается только для выпадающих списков без автодополнения.
   * Для списков с автодополнением нужно использовать aria-activedescendant.
   * @private
   */
  _ariaFocusOnActiveItem: function (el) {
    if (this.options.forAutocomplete) {
      return;
    }
  ...
  _ariaOnShow: function () {
    this.list && attr(this.list, 'aria-expanded', true);
  },
  _ariaOnHide: function () {
    this.list && attr(this.list, 'aria-expanded', false);
  }
});
...
createChildClass('Indexer', UiUtil, {
  ...
  indexItem: function(k, v, additionalKey) {
    ...
    for (i = 0; i < this.options.indexkeys.length; i++) {
      ...
      current_words += ' ' + String(v[this.options.indexkeys[i]]).replace(this.options.delimeter, ' ').replace(/<[^>]*>/g, '').replace(/[\u00AB\u00BB]/g, ''); // «»
    }
...
createChildClass('InlineEdit', UiControl, {
  ...
  '<tbody>{content}\
   <tr>\
    <td class="inlButtonOk"><button class="flat_button button_wide">{yeslabel}</button></td>\
    <td class="inlButtonCancel"><button class="flat_button secondary button_wide">{nolabel}</button></td>\
   </tr></tbody>',
  ...
  initDOM: function(target, options) {
    ...
    this.buttonOk = this.buttonOkCell.firstElementChild;
    this.buttonCancel = this.buttonCancelCell.firstElementChild;
  ...
  initEvents: function() {
    ...
    if (!this.options.notAddClickEvent) {
...
/*
  ...
  withArrow
  selected
  popupClass - custom class on .idd_popup element
*/
function InlineDropdown(el, opts) {
  ...
  this._iddEl = se('<div class="idd_wrap" id="' + el.id + '">' +
    '<div class="idd_selected_value ' + (opts.withArrow ? 'idd_arrow' : '') + '" tabindex="0" role="button">' + el.innerHTML + '</div>' +
  ...
  if (this._opts.autoShow) {
    var onDD = false;
    const onActiveHandler = function() {
      onDD = true;
    ...
    const onDeactiveHandler = function() {
      clearTimeout(_this._autoShowTimeout);
      onDD = false;
    };
    addEvent(this._iddEl, 'mouseenter', onActiveHandler);
    addEvent(this._iddEl, 'mouseleave', onDeactiveHandler);
    addEvent(this._iddEl, 'keydown', (event) =>
      event.key === 'Enter' && event.target.getAttribute('class').includes('idd_selected_value') ? onActiveHandler() : null
    );
  } else {
    addEvent(this._iddEl, 'click', this._onClick.bind(this));
    addEvent(this._iddEl, 'keydown', (event) =>
      event.key === 'Enter' && event.target.getAttribute('class').includes('idd_selected_value') ? this._onClick.call(this) : null
    );
  }
...
InlineDropdown.prototype._rebuildDropdown = function() {
  ...
  var ariaLabel = this._opts.ariaLabel || '';
  ...
  var itemsHTML = '<div class="idd_items_wrap">' +
                    '<div class="idd_items_content" role="list" aria-label="' + ariaLabel + '">';
...
InlineDropdown.prototype._onClick = function(event) {
  ...
  if (this._opts.popupClass) {
    addClass(popupEl, this._opts.popupClass);
  }
...
InlineDropdown.prototype._onKey = function(code, event) {
  if (code == 38 || code == 40) {
    ...
    let _this = this;
    let itemsContentEl = geByClass1('idd_items_content', this._els.popupItems)
    let hoveredItemEl = event.target;
    if (event.target.getAttribute('class').includes('idd_item')) {
      if (event.shiftKey) {
        hoveredItemEl = hoveredItemEl.previousSibling;
      } else {
        hoveredItemEl = hoveredItemEl.nextSibling;
      ...
      hoveredItemEl = itemsContentEl.lastChild;
    } else {
      hoveredItemEl = itemsContentEl.firstChild;
    }
    _this._hoverItem(hoveredItemEl, true);
    return false;
  } else if (code == 13) {
